...
prerequisites.forEach(({ command, name, required }) => {
  if (commandExists(command)) {
    ...
    if (required) {
      ...
      console.log(`  ${name} is not installed (optional)`);
    }
  }
});
...
try {
  ...
  if (majorVersion < 18) {
    console.log('  WARNING: Node.js 18+ is recommended for this project');
  }
} catch (error) {
...
if (process.platform === 'darwin') {
  ...
  if (fs.existsSync('ios')) {
    if (commandExists('pod')) {
      ...
      console.log('  CocoaPods not found. Install with: sudo gem install cocoapods');
    }
  } else {
...
if (androidHome && fs.existsSync(androidHome)) {
  ...
  console.log('  ANDROID_HOME not set or directory missing');
...
try {
  ...
  if (gitUser && gitEmail) {
    ...
    console.log('  Git user not configured. Please run:');
    console.log('   git config --global user.name "Your Name"');
    console.log('   git config --global user.email "your.email@example.com"');
  }
} catch (error) {
  console.log('  Git not configured. Please set up your git user and email');
}
...
try {
  ...
  console.log('  Not logged in to Expo. Run: expo login');
}
...
if (fs.existsSync('scripts/debug-check.js')) {
  ...
  console.log('  Debug check script not found');
}
