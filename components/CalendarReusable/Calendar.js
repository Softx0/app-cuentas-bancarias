...
export class Calendar extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      canBeSameDay: props.canBeSameDay,
      styles: {},
  ...
  static defaultProps = {
    ...
    nextTitle: "Next",
    selectMonthTitle: "Selecciona el mes del ",
    selectYearTitle: "Seleciona el año",
    horizontal: true,
  ...
  componentDidUpdate(prevProps) {
    ...
    if (doStateUpdate) {
      ...
      const renderMonthParams = this.createMonthProps(_state);
  ...
  updateScaledStyles = (props) => {
    const { scaleFactor, selectedDayColor, selectedDayTextColor, todayBackgroundColor, width, height, dayShape } = props;
  ...
  updateMonthYear = (updateState, initialDate = this.props.initialDate) => {
    const newState = {
    ...
    if (_minRangeDuration) {
      if (Array.isArray(_minRangeDuration)) {
        _minRangeDuration.map((mrd) => {
          const thisDate = new Date(mrd.date);
  ...
  handleOnPressDay = ({ year, month, day }) => {
    const { selectedStartDate: prevSelectedStartDate, selectedEndDate: prevSelectedEndDate } = this.state;

    const { allowRangeSelection, allowBackwardRangeSelect, enableDateChange, onDateChange } = this.props;
    ...
    if (allowRangeSelection && prevSelectedStartDate && !prevSelectedEndDate) {
      ...
      const syncEndDate = Boolean(prevSelectedEndDate);
      const selectedStartDate = date;
  ...
  createMonths = (props, { currentMonth, currentYear }) => {
    ...
    const monthsList = [];
    const numMonths = this.numMonthsScroll;
    ...
    for (let i = 0; i < numMonths; i++) {
      const month = addMonths(firstScrollerMonth, i);
  ...
  render() {
    const { currentView, currentMonth, currentYear, minDate, maxDate, styles, monthsList, renderMonthParams, initialScrollerIndex } = this.state;
      ...
      default:
        content = (
          <View styles={styles.calendar}>
            ...
            />
            {/* Mostrar inputs de rango de fechas si está habilitado y hay dos fechas seleccionadas */}
            {enableDateRangeInputs ? (
              <DateRangeInputs
                startDate={this.state.selectedStartDate}
                endDate={this.props.canBeSameDay && !this.state.selectedEndDate ? this.state.selectedStartDate : this.state.selectedEndDate}
                onStartDateChange={(date) => this.props.onDateChange(date, Utils.START_DATE)}
                onEndDateChange={(date) => this.props.onDateChange(date, Utils.END_DATE)}
                minDate={minDate}
                maxDate={maxDate}
              />
            ) : null}
            ...
            <View style={{ flexDirection: "row", gap: 12, marginTop: 20, borderTopWidth: 1, borderTopColor: "#D7DAE0", paddingTop: 16 }}>
              <ButtonCalendarReusable
                label={"Cancelar"}
                style="rounded"
                hierarchy="secondaryGray"
                // Se elimino el uso de EVENTS y EventEmitter para simplificacion de la complejidad y adopcion de la celula de App Intermediarios
                // onPress={() => HumanoEventEmitter.emit(EVENT.TOGGLE_CUSTOM_MODAL)}
              ...
              <ButtonCalendarReusable
                ...
                // disabled={
                //   disabledButton ||
                //   (this.state.selectedStartDate && this.state.selectedEndDate && isSameDay(this.state.selectedStartDate, this.state.selectedEndDate))
                // }
                disabled={disabledButton || (!this.state.canBeSameDay && isSameDay(this.state.selectedStartDate, this.state.selectedEndDate))}
                containerStyle={{ flex: 1 }}
              />
            </View>
          </View>
