...
export default class DaysGridView extends Component {
          ...
          for (let i = 0; i < daysGrid.length; i++) {
            for (let j = 0; j < daysGrid[i].length; j++) {
              ...
              if (isSelected || isPrevSelected) {
                daysGrid[i][j] = this.renderDayInCurrentMonth(day, j === 0, j === daysGrid[i].length - 1);
              }
            }
          }
  ...
  renderDayInCurrentMonth(day, firstElementInRow, lastElementInRow) {
    const props = { ...this.props, day, key: day, firstElementInRow, lastElementInRow };
        ...
        if (i === 0) {
          ...
          if (j >= startIndex) {
            if (dayOfMonth <= numDaysInMonth) {
              daysGrid[i].push(this.renderDayInCurrentMonth(dayOfMonth++, j === 0, j === numDaysInWeek - 1));
            }
          } else {
          ...
          lastFilledRow = i;
          daysGrid[i].push(this.renderDayInCurrentMonth(dayOfMonth++, j === 0, j === numDaysInWeek - 1));
        } else if (this.props.showDayStragglers && i <= lastFilledRow) {
