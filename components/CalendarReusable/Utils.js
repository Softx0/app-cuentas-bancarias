...
export const Utils = {
  ...
  isSameMonthAndYear(date, month, year) {
    if (date) {
  ...
  compareDates(a, b, granularity) {
    ...
    if (granularity === "day") {
      return Boolean(a) && Boolean(b) && isSameDay(a, b);
    }

    if (granularity === "month") {
      return Boolean(a) && Boolean(b) && isSameMonth(a, b);
    }

    return true;
  },
  getWeekdays(firstDay = 0) {
    let from = firstDay;
    const weekdays = [];
  ...
  getISOWeekdaysOrder(firstDay = 0) {
